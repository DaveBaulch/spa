<template>
  <section class="section carousel-block">
    ...
    <button @click="togglePause()" :class="isPaused ? 'carousel-play-btn.is-paused' : 'carousel-play-btn'" role="button" aria-label="Pause carousel">Pause</button>
...
<script>
  ...
  methods: {
    togglePause() {
      ...
      if (this.isPaused) {
        this.$refs.carousel.slickPlay()
      } else {
        this.$refs.carousel.slickPause()
      }
...
.carousel-play-btn.is-paused {
  background: transparent url('~@/assets/images/svg/icon-play.svg') no-repeat center center;
}
